...
import { MOCK_SAMPLES, MOCK_MEMBERS } from "@/lib/mock-data";
...
/* ─── Supabase에서 applications 데이터 fetch ─── */
function useApplications() {
  ...
  const fetchApps = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) { const d = await res.json(); setError(d.error || `조회 실패 (${res.status})`); return; }
      const d = await res.json();
      setApps(d.data || []);
    } catch (e) { setError("네트워크 오류: " + e.message); } finally { setLoading(false); }
  }, []);

  useEffect(() => { fetchApps(); }, [fetchApps]);

  const updateLocal = (id, updates) => setApps((p) => p.map((a) => (a.id === id ? { ...a, ...updates } : a)));

  const updateRemote = async (id, updates) => {
    updateLocal(id, updates);
    try {
      await fetch("/api/applications", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id, updates }) });
...
/* ─── Dashboard ─── */
function Dashboard({ apps, onNavigate, onSelectApp, onSelectSample }) {
  ...
  return (<div><h1 className="text-2xl font-extrabold text-black mb-6">관리자 대시보드</h1>
    <div className="grid grid-cols-4 gap-3.5 mb-9">
      {[{ label: "신규 크리에이터", value: pending, accent: true }, { label: "전체 신청", value: apps.length }, { label: "승인됨", value: apps.filter((a) => a.status === "approved").length }, { label: "샘플 진행", value: MOCK_SAMPLES.length }].map((s, i) => (<div key={i} className="p-5 rounded-xl bg-white border border-gray-100"><div className="text-[11px] text-gray-400 font-semibold mb-1.5">{s.label}</div><div className={`text-[28px] font-black ${s.accent ? "text-[#FFD600]" : "text-black"}`}>{s.value}</div></div>))}
    </div>
    <div className="grid grid-cols-2 gap-5">
      <div><div className="flex items-center justify-between mb-3"><h2 className="text-base font-bold">최근 크리에이터 신청</h2><button onClick={() => onNavigate("applications")} className="text-[11px] text-gray-400 bg-transparent border-none cursor-pointer hover:text-black">전체 보기 →</button></div>
        {apps.length === 0 ? <p className="text-[13px] text-gray-300 p-4">신청 내역 없음</p> : apps.slice(0, 5).map((a) => (<div key={a.id} onClick={() => { onNavigate("applications"); setTimeout(() => onSelectApp?.(a), 50); }} className="px-4 py-3 rounded-lg border border-gray-100 mb-2 bg-white flex justify-between items-center cursor-pointer hover:border-[#FFD600] hover:shadow-sm transition-all group"><div className="flex items-center gap-2"><span className="text-sm font-bold">{a.artist_name}</span>{a.isVipCreator && <VipBadge />}</div><div className="flex items-center gap-2"><StatusBadge status={a.status} type="app" /><svg width="14" height="14" viewBox="0 0 20 20" fill="none" stroke="#ccc" strokeWidth="2" strokeLinecap="round" className="opacity-0 group-hover:opacity-100"><path d="M7 4L13 10L7 16"/></svg></div></div>))}
      </div>
      <div><div className="flex items-center justify-between mb-3"><h2 className="text-base font-bold">최근 샘플</h2><button onClick={() => onNavigate("samples")} className="text-[11px] text-gray-400 bg-transparent border-none cursor-pointer hover:text-black">전체 보기 →</button></div>{MOCK_SAMPLES.slice(0, 3).map((s) => (<div key={s.id} onClick={() => { onNavigate("samples"); setTimeout(() => onSelectSample?.(s), 50); }} className="px-4 py-3 rounded-lg border border-gray-100 mb-2 bg-white flex justify-between items-center cursor-pointer hover:border-[#FFD600] hover:shadow-sm transition-all group"><div><span className="text-[13px] font-bold">{s.product_description}</span></div><StatusBadge status={s.status} /></div>))}</div>
    </div></div>);
}

/* ─── Applications Manager (Supabase 연동, 더미 없음) ─── */
function ApplicationsManager({ apps, updateLocal, updateRemote, initialDetail, fetchApps }) {
  ...
  useEffect(() => { if (detail) { const f = apps.find((a) => a.id === detail.id); if (f) setDetail(f); } }, [apps]);

  const toggleVip = (id) => { const a = apps.find((x) => x.id === id); if (a) updateRemote(id, { isVipCreator: !a.isVipCreator }); };

  const handleAction = async (app, action) => { if (action === "approved") { if (!confirm("승인 처리하시겠습니까?")) return; await processAction(app, action, ""); } else { setModal({ app, action }); setAdminNote(""); } };
  const processAction = async (app, action, note) => {
    ...
    try {
      ...
      try {
        ...
        await fetch(`/api/admin/applications/${app.id}/${ep}`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ artist_name: app.artist_name, email: app.email, admin_note: note }) });
      } catch {}
      // DB 상태 변경
      await updateRemote(app.id, { status: action });
      alert({ approved: "승인 완료", rejected: "거절 완료", on_hold: "보류 완료" }[action]);
    } catch { alert("처리 실패"); } finally { setLoading(null); setModal(null); }
  };
  const mc = { rejected: { title: "거절", ph: "거절 사유", btn: "거절 처리", cls: "text-red-700 bg-red-50 border border-red-200" }, on_hold: { title: "보류", ph: "보완 요청", btn: "보류 처리", cls: "text-orange-700 bg-orange-50 border border-orange-200" } };

  if (detail) return (<div>
    <button onClick={() => setDetail(null)} className="text-[13px] text-gray-400 mb-6 bg-transparent border-none cursor-pointer flex items-center gap-1"><svg width="14" height="14" viewBox="0 0 20 20" fill="none" stroke="#999" strokeWidth="2" strokeLinecap="round"><path d="M14 4L6 10L14 16"/></svg>목록으로</button>
  ...
  return (<div>{modal && (<div className="fixed inset-0 z-[9999] bg-black/50 backdrop-blur-sm flex items-center justify-center p-6"><div className="bg-white rounded-2xl w-full max-w-[440px] shadow-2xl overflow-hidden"><div className="px-6 py-5 border-b border-gray-100"><h3 className="text-[17px] font-extrabold">{mc[modal.action].title}</h3><p className="text-xs text-gray-400 mt-1">{modal.app.artist_name}</p></div><div className="px-6 py-5"><textarea value={adminNote} onChange={(e) => setAdminNote(e.target.value)} placeholder={mc[modal.action].ph} rows={4} className="w-full px-3.5 py-3 text-sm border border-gray-200 rounded-lg outline-none resize-y focus:border-[#FFD600]" /></div><div className="px-6 pb-5 flex gap-2.5"><button onClick={() => setModal(null)} className="flex-1 py-3 text-sm font-semibold text-gray-500 bg-gray-100 rounded-xl border-none cursor-pointer">취소</button><button onClick={() => processAction(modal.app, modal.action, adminNote)} disabled={loading === modal.app.id} className={`flex-1 py-3 text-sm font-bold rounded-xl border-none cursor-pointer ${mc[modal.action].cls} ${loading === modal.app.id ? "opacity-60" : ""}`}>{loading === modal.app.id ? "..." : mc[modal.action].btn}</button></div></div></div>)}
    <div className="flex items-center justify-between mb-1"><h1 className="text-2xl font-extrabold text-black">크리에이터 신청 관리</h1><button onClick={fetchApps} className="text-[11px] font-semibold text-gray-400 bg-gray-100 px-3 py-1.5 rounded-lg border-none cursor-pointer hover:bg-gray-200">새로고침</button></div>
    <p className="text-[13px] text-gray-400 mb-6">Supabase applications 테이블 실시간 조회</p>
    <div className="flex gap-2 mb-5">{["all","pending","on_hold","approved","rejected"].map((v) => (<button key={v} onClick={() => setFilter(v)} className={`px-3.5 py-1.5 text-xs font-semibold rounded-full border-none cursor-pointer ${filter === v ? "bg-black text-white" : "bg-gray-100 text-gray-500"}`}>{{all:`전체 (${apps.length})`,pending:`대기 (${apps.filter(a=>a.status==="pending").length})`,on_hold:"보류",approved:"승인",rejected:"거절"}[v]}</button>))}</div>
    {apps.length === 0 ? (<div className="bg-white rounded-xl border border-gray-100 p-12 text-center"><p className="text-sm text-gray-300">신청 내역이 없습니다</p></div>) : (
...
/* ─── Samples (기존 mock 유지 — 별도 테이블 연동 시 교체) ─── */
function SamplesManager({ initialDetail }) {
  ...
  const [tracking, setTracking] = useState(""); const [trackLoading, setTrackLoading] = useState(false); const [trackResult, setTrackResult] = useState(null); const [trackError, setTrackError] = useState("");
  ...
  const handleShip = () => { if (!tracking) return alert("운송장 번호를 입력하세요."); const dl = calcPromoDeadline(new Date().toISOString()); updateSample(detail.id, { courierName: COURIER_NAME, trackingNumber: tracking, shippedAt: new Date().toISOString(), status: "shipped", deliveryStatus: "shipped", promoDeadline: dl }); alert("출고 완료!"); };
  const handleTrack = async (s) => { const t = s || detail; if (!t?.trackingNumber || t.deliveryConfirmedByAdmin) return; setTrackLoading(true); setTrackError(""); setTrackResult(null); try { const r = await fetch(`/api/delivery?tracking=${encodeURIComponent(t.trackingNumber)}`); const d = await r.json(); if (!r.ok) { setTrackError(d.error); return; } setTrackResult(d); if (!t.deliveryConfirmedByAdmin) updateSample(t.id, { deliveryStatus: d.status, status: d.status === "delivered" ? "delivered" : t.status, lastCheckedAt: new Date().toISOString() }); } catch { setTrackError("네트워크 오류"); } finally { setTrackLoading(false); } };
  const handleDeliveryConfirm = () => { if (!confirm("배송완료 수동 처리?")) return; updateSample(detail.id, { deliveryStatus: "delivered", status: "delivered", deliveredAt: new Date().toISOString(), deliveryConfirmedByAdmin: true }); alert("배송완료 처리됨"); };
  ...
  if (detail) { const promo = getPromoStatus(detail); const evPromo = getEventPromoStatus(detail); const blocked = shouldBlock(detail); return (<div>
    <button onClick={() => { setDetail(null); setTrackResult(null); }} className="text-[13px] text-gray-400 mb-6 bg-transparent border-none cursor-pointer flex items-center gap-1"><svg width="14" height="14" viewBox="0 0 20 20" fill="none" stroke="#999" strokeWidth="2" strokeLinecap="round"><path d="M14 4L6 10L14 16"/></svg>목록으로</button>
    <div className="bg-white rounded-xl border border-gray-100 p-6 mb-5">
      ...
      <div className="p-4 rounded-lg bg-[#fafaf6] border border-[rgba(255,214,0,0.15)] mb-4"><div className="text-[11px] text-gray-400 font-semibold mb-2">홍보</div><div className="grid grid-cols-2 gap-2"><div className="p-2 rounded bg-white"><div className="text-[10px] text-gray-400">온라인</div><div className="text-[12px] font-bold mt-0.5" style={{ color: promo.color }}>{promo.text}</div></div><div className="p-2 rounded bg-white"><div className="text-[10px] text-gray-400">행사</div><div className="text-[12px] font-bold mt-0.5" style={{ color: evPromo.color }}>{evPromo.text}{detail.eventPromoSubmitted && " 🔴"}</div></div></div>{blocked && <div className="mt-3 flex items-center justify-between p-2.5 rounded-lg bg-red-50"><span className="text-[12px] font-bold text-[#c62828]">⚠ 차단됨</span><button onClick={() => updateSample(detail.id, { adminUnlocked: true })} className="text-[11px] font-bold text-white bg-[#c62828] px-3 py-1 rounded-md border-none cursor-pointer">해제</button></div>}</div>
      <div className="p-4 rounded-lg bg-[#f8f8f8] mb-4"><div className="text-[11px] text-gray-400 font-semibold mb-2">📦 배송 정보</div>{detail.shipping ? (<div className="grid grid-cols-2 gap-2">{[{ l:"수령인", v:detail.shipping.recipient }, { l:"연락처", v:detail.shipping.phone }, { l:"주소", v:`${detail.shipping.address} ${detail.shipping.address_detail||""}` }].map((i, idx) => (<div key={idx} className="p-2 rounded bg-white"><div className="text-[10px] text-gray-400">{i.l}</div><div className="text-[12px] font-semibold text-gray-700 mt-0.5">{i.v}</div></div>))}</div>) : <p className="text-[12px] text-gray-400">미입력</p>}</div>
      {detail.status === "shipping_ready" && (<div className="p-4 rounded-xl border-2 border-[#FFD600] bg-[rgba(255,214,0,0.03)] mb-4"><h3 className="text-[13px] font-bold mb-3">출고 처리</h3><div className="mb-3"><div className="flex items-center gap-2 mb-2"><span className="text-[11px] text-gray-500">택배사</span><span className="text-[13px] font-bold">{COURIER_NAME}</span></div><input value={tracking} onChange={(e) => setTracking(e.target.value)} placeholder="우체국 운송장 번호" className="w-full px-3 py-2.5 text-[13px] border border-gray-200 rounded-lg outline-none bg-white focus:border-[#FFD600]" /></div><button onClick={handleShip} disabled={!tracking} className={`w-full py-2.5 text-[13px] font-bold rounded-lg border-none ${tracking ? "text-black bg-[#FFD600] cursor-pointer" : "text-gray-400 bg-gray-100 cursor-not-allowed"}`}>출고 처리</button></div>)}
      {detail.trackingNumber && (<div className="p-4 rounded-lg bg-[#f8f8f8] mb-4"><div className="flex items-center justify-between mb-3"><div className="text-[11px] text-gray-400 font-semibold">🚚 배송 추적{detail.deliveryConfirmedByAdmin && <span className="ml-1.5 text-[10px] font-bold text-green-700 bg-green-50 px-2 py-0.5 rounded-full">관리자 확정</span>}</div><div className="flex items-center gap-2"><DeliveryBadge status={detail.deliveryStatus} />{detail.deliveryStatus !== "delivered" && <button onClick={() => handleTrack(detail)} disabled={trackLoading} className={`text-[11px] font-bold px-3 py-1 rounded-md border-none cursor-pointer ${trackLoading ? "text-gray-400 bg-gray-100" : "text-[#1565c0] bg-[#e3f2fd]"}`}>{trackLoading ? "..." : "배송 조회"}</button>}{detail.deliveryStatus !== "delivered" && <button onClick={handleDeliveryConfirm} className="text-[11px] font-bold px-3 py-1 rounded-md border-none cursor-pointer text-white bg-[#2e7d32]">배송완료</button>}</div></div>
        <div className="grid grid-cols-2 gap-2 mb-2"><div className="p-2 rounded bg-white"><div className="text-[10px] text-gray-400">택배사</div><div className="text-[12px] font-semibold text-gray-700 mt-0.5">{COURIER_NAME}</div></div><div className="p-2 rounded bg-white"><div className="text-[10px] text-gray-400">운송장</div><div className="text-[12px] font-semibold text-gray-700 mt-0.5">{detail.trackingNumber}</div></div></div>
        {trackError && <div className="p-2 rounded-lg bg-red-50 text-[12px] text-red-700 mb-2">{trackError}</div>}
        {trackResult?.details && (<div className="border border-gray-100 rounded-lg overflow-hidden">{trackResult.details.slice().reverse().map((d, i) => (<div key={i} className={`px-3 py-2 text-[11px] flex gap-3 ${i > 0 ? "border-t border-gray-50" : ""}`}><span className="text-gray-400 w-[110px] flex-shrink-0">{d.time}</span><span className="text-gray-700 font-medium">{d.location}</span><span className="text-gray-500">{d.detail}</span></div>))}</div>)}</div>)}
      {ns[detail.status]?.length > 0 && detail.status !== "shipping_ready" && (<div className="flex gap-2">{ns[detail.status].map((n) => (<button key={n} onClick={() => updateSample(detail.id, { status: n })} className={`px-4 py-2 text-[12px] font-bold rounded-lg border-none cursor-pointer ${n === "cancelled" ? "text-red-700 bg-red-50" : "text-black bg-[#FFD600]"}`}>{STATUS_MAP[n]?.label || n}</button>))}</div>)}
    </div></div>); }
  ...
  return (<div><h1 className="text-2xl font-extrabold text-black mb-6">샘플 관리</h1>
    ...
    <div className="bg-white rounded-xl border border-gray-100 overflow-hidden"><table className="w-full text-[13px] border-collapse"><thead><tr className="bg-[#fafafa]">{["샘플번호","설명","배송","상태"].map((h) => <th key={h} className="px-3.5 py-3 text-left font-semibold text-gray-400 border-b border-gray-100">{h}</th>)}</tr></thead>
    <tbody>{filtered.map((s) => (<tr key={s.id} className="border-b border-gray-50 hover:bg-gray-50/50 cursor-pointer" onClick={() => { setDetail(s); setTracking(s.trackingNumber || ""); setTrackResult(null); setTrackError(""); }}>
      <td className="px-3.5 py-3 font-semibold text-gray-700 text-xs">{s.sample_number}</td>
      <td className="px-3.5 py-3 text-gray-500 truncate max-w-[200px]">{s.product_description}</td>
...
export default function AdminPage() {
  ...
  const { apps, loading: appsLoading, error: appsError, fetchApps, updateLocal, updateRemote } = useApplications();

  if (checking) return (<div className="min-h-screen bg-[#f8f8f6] flex items-center justify-center"><div className="w-10 h-10 bg-[#FFD600] rounded-lg flex items-center justify-center mx-auto mb-4 animate-pulse"><span className="font-extrabold text-base text-black">F</span></div></div>);
  if (!user) return null;

  const nav = (v) => { setView(v); setPa(null); setPs(null); };
  const r = () => {
    switch (view) {
      case "applications": return appsLoading ? <div className="py-12 text-center text-sm text-gray-400">로딩 중...</div> : appsError ? <div className="py-12 text-center"><p className="text-sm text-red-500 mb-3">{appsError}</p><button onClick={fetchApps} className="text-sm text-[#1565c0] bg-[#e3f2fd] px-4 py-2 rounded-lg border-none cursor-pointer">다시 시도</button></div> : <ApplicationsManager apps={apps} updateLocal={updateLocal} updateRemote={updateRemote} initialDetail={pa} fetchApps={fetchApps} />;
      case "samples": return <SamplesManager initialDetail={ps} />;
      case "members": return <MembersManager />;
      default: return <Dashboard apps={apps} onNavigate={nav} onSelectApp={setPa} onSelectSample={setPs} />;
    }
  };
