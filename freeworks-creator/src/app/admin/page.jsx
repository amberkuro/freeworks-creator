...
function useApplications() {
  ...
  const fetchApps = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) { setError(data.error || "데이터 조회 실패"); return; }
      setApps(data.data || []);
  ...
  const updateRemote = async (id, updates) => {
    ...
    } catch (err) { console.error("[업데이트 에러]", err); }
  };
...
function Dashboard({ apps, onNavigate, onSelectApp, onSelectSample }) {
  const stats = [{ label: "신규 크리에이터", value: apps.filter((a) => a.status === "pending").length, accent: true }, { label: "제작/배송 진행", value: MOCK_SAMPLES.filter((s) => ["in_production","shipping_ready","shipped","in_transit"].includes(s.status)).length }, { label: "홍보 미완료", value: MOCK_SAMPLES.filter((s) => s.shippedAt && !s.promoSubmitted).length }, { label: "행사 홍보 완료", value: MOCK_SAMPLES.filter((s) => s.eventPromoSubmitted).length }];
...
/* ─── Applications Manager (Supabase 연동) ─── */
function ApplicationsManager({ apps, updateLocal, updateRemote, initialDetail, fetchApps }) {
  ...
  // detail 동기화
  useEffect(() => { if (detail) { const fresh = apps.find((a) => a.id === detail.id); if (fresh) setDetail(fresh); } }, [apps]);

  const toggleVip = async (id) => { const a = apps.find((x) => x.id === id); if (a) await updateRemote(id, { isVipCreator: !a.isVipCreator }); };
  const saveMemo = async (id, memo) => { await updateRemote(id, { adminMemo: memo }); };
  ...
  const processAction = async (app, action, note) => {
    ...
    setLoading(app.id);
    try {
      const r = await fetch(`/api/admin/applications/${app.id}/${ep}`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ artist_name: app.artist_name, email: app.email, admin_note: note }) });
      const d = await r.json();
      if (d.success) {
        await updateRemote(app.id, { status: action });
        alert({ approved: "승인 완료", rejected: "거절 완료", on_hold: "보류 완료" }[action]);
      } else alert("오류: " + (d.error || ""));
    } catch { alert("네트워크 오류"); } finally { setLoading(null); setModal(null); }
  };
  ...
  return (<div>{modal && (<div className="fixed inset-0 z-[9999] bg-black/50 backdrop-blur-sm flex items-center justify-center p-6"><div className="bg-white rounded-2xl w-full max-w-[440px] shadow-2xl overflow-hidden"><div className="px-6 py-5 border-b border-gray-100"><h3 className="text-[17px] font-extrabold">{mc[modal.action].title}</h3><p className="text-xs text-gray-400 mt-1">{modal.app.artist_name}</p></div><div className="px-6 py-5"><p className="text-[13px] text-gray-500 mb-3">{mc[modal.action].desc}</p><textarea value={adminNote} onChange={(e) => setAdminNote(e.target.value)} placeholder={mc[modal.action].ph} rows={4} className="w-full px-3.5 py-3 text-sm border border-gray-200 rounded-lg outline-none resize-y focus:border-[#FFD600]" /></div><div className="px-6 pb-5 flex gap-2.5"><button onClick={() => setModal(null)} className="flex-1 py-3 text-sm font-semibold text-gray-500 bg-gray-100 rounded-xl border-none cursor-pointer">취소</button><button onClick={() => processAction(modal.app, modal.action, adminNote)} disabled={loading === modal.app.id} className={`flex-1 py-3 text-sm font-bold rounded-xl border-none cursor-pointer ${mc[modal.action].cls} ${loading === modal.app.id ? "opacity-60" : ""}`}>{loading === modal.app.id ? "..." : mc[modal.action].btn}</button></div></div></div>)}
    <div className="flex items-center justify-between mb-1">
      ...
      <button onClick={fetchApps} className="text-[11px] font-semibold text-gray-400 bg-gray-100 px-3 py-1.5 rounded-lg border-none cursor-pointer hover:bg-gray-200">새로고침</button>
    </div>
    <p className="text-[13px] text-gray-400 mb-6">Supabase applications 테이블에서 실시간 조회 · 행 클릭 → 상세</p>
    <div className="flex gap-2 mb-5">{["all","pending","on_hold","approved","rejected"].map((v) => (<button key={v} onClick={() => setFilter(v)} className={`px-3.5 py-1.5 text-xs font-semibold rounded-full border-none cursor-pointer ${filter === v ? "bg-black text-white" : "bg-gray-100 text-gray-500"}`}>{{all:`전체 (${apps.length})`,pending:`검토 대기 (${apps.filter(a=>a.status==="pending").length})`,on_hold:"보류",approved:"승인",rejected:"거절"}[v]}</button>))}</div>
    {apps.length === 0 ? (<div className="bg-white rounded-xl border border-gray-100 p-12 text-center"><p className="text-sm text-gray-300">아직 신청 내역이 없습니다</p></div>) : (
...
function SamplesManager({ initialDetail }) {
  ...
  if (detail) { const promo = getPromoStatus(detail); const evPromo = getEventPromoStatus(detail); const blocked = shouldBlock(detail); return (<div>
    ...
    <div className="bg-white rounded-xl border border-gray-100 p-6 mb-5">
      ...
      <div className="grid grid-cols-3 gap-3 mb-5">{[{ l:"제품", v:detail.product_category==="acrylic"?"아크릴":"DTF" }, { l:"데이터", v:detail.data_type==="artwork_only"?"그림만":"완성" }, { l:"신청일", v:detail.created_at }].map((i, idx) => (<div key={idx} className="p-3 rounded-lg bg-[#f8f8f8]"><div className="text-[11px] text-gray-400 font-semibold mb-1">{i.l}</div><div className="text-[13px] font-semibold text-gray-700">{i.v}</div></div>))}</div>
      <div className="p-4 rounded-lg bg-[#fafaf6] border border-[rgba(255,214,0,0.15)] mb-4"><div className="text-[11px] text-gray-400 font-semibold mb-2">홍보 관리</div><div className="grid grid-cols-2 gap-2"><div className="p-2 rounded bg-white"><div className="text-[10px] text-gray-400 font-semibold">온라인 홍보</div><div className="text-[12px] font-bold mt-0.5" style={{ color: promo.color }}>{promo.text}</div></div><div className="p-2 rounded bg-white"><div className="text-[10px] text-gray-400 font-semibold">행사장 홍보</div><div className="flex items-center gap-1 mt-0.5"><span className="text-[12px] font-bold" style={{ color: evPromo.color }}>{evPromo.text}</span>{detail.eventPromoSubmitted && <span className="text-[10px]">🔴</span>}</div></div></div>{blocked && (<div className="mt-3 flex items-center justify-between p-2.5 rounded-lg bg-red-50 border border-red-100"><span className="text-[12px] font-bold text-[#c62828]">⚠ 자동 차단됨</span><button onClick={() => handleUnblock(detail.id)} className="text-[11px] font-bold text-white bg-[#c62828] px-3 py-1 rounded-md border-none cursor-pointer">차단 해제</button></div>)}</div>
      <div className="p-4 rounded-lg bg-[#f8f8f8] mb-4"><div className="text-[11px] text-gray-400 font-semibold mb-2 flex items-center justify-between"><span>📦 배송 정보</span>{detail.shipping ? <span className="text-[10px] font-bold text-green-700 bg-green-50 px-2 py-0.5 rounded-full">입력 완료</span> : <span className="text-[10px] font-bold text-[#0277bd] bg-[#e1f5fe] px-2 py-0.5 rounded-full">미입력</span>}</div>{detail.shipping ? (<div className="grid grid-cols-2 gap-2">{[{ l:"수령인", v:detail.shipping.recipient }, { l:"연락처", v:detail.shipping.phone }, { l:"우편번호", v:detail.shipping.zipcode }, { l:"주소", v:`${detail.shipping.address} ${detail.shipping.address_detail||""}`.trim() }].map((i, idx) => (<div key={idx} className="p-2 rounded bg-white"><div className="text-[10px] text-gray-400 font-semibold">{i.l}</div><div className="text-[12px] font-semibold text-gray-700 mt-0.5">{i.v}</div></div>))}</div>) : <p className="text-[12px] text-gray-400">미입력</p>}</div>
      {detail.status === "shipping_ready" && (<div className="p-4 rounded-xl border-2 border-[#FFD600] bg-[rgba(255,214,0,0.03)] mb-4"><h3 className="text-[13px] font-bold text-black mb-3">출고 처리</h3><div className="mb-3"><div className="flex items-center gap-2 mb-2"><span className="text-[11px] font-semibold text-gray-500">택배사</span><span className="text-[13px] font-bold text-black">{COURIER_NAME}</span></div><label className="block text-[11px] font-semibold text-gray-500 mb-1">운송장 번호</label><input value={tracking} onChange={(e) => setTracking(e.target.value)} placeholder="우체국 운송장 번호" className="w-full px-3 py-2.5 text-[13px] border border-gray-200 rounded-lg outline-none bg-white focus:border-[#FFD600]" /></div><button onClick={handleShip} disabled={!tracking} className={`w-full py-2.5 text-[13px] font-bold rounded-lg border-none ${tracking ? "text-black bg-[#FFD600] cursor-pointer" : "text-gray-400 bg-gray-100 cursor-not-allowed"}`}>출고 처리</button></div>)}
      {detail.trackingNumber && (<div className="p-4 rounded-lg bg-[#f8f8f8] mb-4"><div className="flex items-center justify-between mb-3"><div className="text-[11px] text-gray-400 font-semibold">🚚 배송 추적 (우체국){detail.deliveryConfirmedByAdmin && <span className="ml-1.5 text-[10px] font-bold text-green-700 bg-green-50 px-2 py-0.5 rounded-full">관리자 확정</span>}</div><div className="flex items-center gap-2"><DeliveryBadge status={detail.deliveryStatus} />{detail.deliveryStatus !== "delivered" && <button onClick={() => handleTrack(detail)} disabled={trackLoading} className={`text-[11px] font-bold px-3 py-1 rounded-md border-none cursor-pointer ${trackLoading ? "text-gray-400 bg-gray-100" : "text-[#1565c0] bg-[#e3f2fd]"}`}>{trackLoading ? "조회 중..." : "배송 조회"}</button>}{detail.deliveryStatus !== "delivered" && <button onClick={handleDeliveryConfirm} className="text-[11px] font-bold px-3 py-1 rounded-md border-none cursor-pointer text-white bg-[#2e7d32] hover:bg-[#1b5e20]">배송완료 처리</button>}</div></div>
        ...
        {trackResult?.details && (<div className="border border-gray-100 rounded-lg overflow-hidden">{trackResult.details.slice().reverse().map((d, i) => (<div key={i} className={`px-3 py-2 text-[11px] flex gap-3 ${i > 0 ? "border-t border-gray-50" : ""}`}><span className="text-gray-400 flex-shrink-0 w-[110px]">{d.time}</span><span className="text-gray-700 font-medium">{d.location}</span><span className="text-gray-500">{d.detail}</span></div>))}</div>)}</div>)}
...
export default function AdminPage() {
  ...
  const r = () => {
    switch (view) {
      case "applications": return appsLoading ? <div className="text-center py-12"><p className="text-sm text-gray-400">데이터 로딩 중...</p></div> : appsError ? <div className="text-center py-12"><p className="text-sm text-red-500">{appsError}</p><button onClick={fetchApps} className="mt-3 text-sm text-[#1565c0] bg-[#e3f2fd] px-4 py-2 rounded-lg border-none cursor-pointer">다시 시도</button></div> : <ApplicationsManager apps={apps} updateLocal={updateLocal} updateRemote={updateRemote} initialDetail={pa} fetchApps={fetchApps} />;
